#!/usr/bin/env python3
"""
boruta_1min_clean.py

Boruta feature selection for 1-MINUTE BTC prediction - OHLCV features only.

Optimized for RL trading:
...
INPUT_FILE = "/home/jarred/git/ServoTrader/data/btc_1min_features_engineered.csv"
OUTPUT_REPORT = "/home/jarred/git/ServoTrader/data/boruta_1min_report.csv"
OUTPUT_SELECTED = "/home/jarred/git/ServoTrader/data/btc_1min_features_selected.csv"

# ============================================================================
# TARGET SELECTION
# ============================================================================
# For RL: Use binary classification target for Boruta
# Actual RL will use continuous returns for rewards

TARGET_VARIABLE = "target_direction_5min"  # Binary: up=1, down=0

# Alternative targets you could use:
# TARGET_VARIABLE = "target_optimal_action_5min"  # Multi-class: BUY/HOLD/SELL

# ============================================================================
# BORUTA PARAMETERS (MODERATE - Good for 1-min)
# ============================================================================

BORUTA_ALPHA = 0.05              # Strict (we have lots of data)
...
RF_N_ESTIMATORS = 250
RF_MAX_DEPTH = 8                 # Moderate
RF_MIN_SAMPLES_SPLIT = 20        # Prevent overfitting
RF_MIN_SAMPLES_LEAF = 10         # Prevent overfitting
RF_MAX_FEATURES = 'sqrt'
RF_RANDOM_STATE = 42

# ============================================================================
# SAMPLING (CRITICAL for 1-min data!)
# ============================================================================
# 1-min data has millions of rows - must sample

SAMPLE_SIZE = 100000             # 100k samples
SAMPLE_RANDOM = True             # Random sample vs recent only

# ============================================================================
# PRE-FILTERING
# ============================================================================

PREFILTER_TOP_N = 100            # Test top 100 features only (faster)
...
FORWARD_FILL_LIMIT = 1440        # 1 day in minutes
...
def categorize_feature(feature_name):
    ...
    # Cyclical
    if any(kw in name_lower for kw in ['minute', 'hour', 'day_of', 'trading', 'weekend']):
        return 'cyclical'
...
def load_and_sample_data():
    """Load and sample 1-min data."""
    
    section("LOADING 1-MINUTE DATA")
    ...
    print(f"  💡 1-min data is HUGE - must sample for Boruta")
    ...
    if len(df) > SAMPLE_SIZE:
        ...
        print(f"  ℹ️  Dataset already small ({len(df):,} rows) - no sampling needed")
    
    section("PREPARING FEATURES")
    
    # Identify feature columns (exclude targets and timestamp)
...
def prefilter_features(df, feature_cols):
    """Pre-filter to top N features by RF importance."""
    ...
    print(f"     (Makes Boruta much faster)")
...
def run_boruta_with_diagnostics(df, feature_cols):
    ...
    if scores.mean() > 0.55:
        print(f"     ✅✅ Excellent signal for 1-min!")
    elif scores.mean() > 0.53:
        print(f"     ✅ Good signal for 1-min")
    elif scores.mean() > 0.52:
        print(f"     ✅ Weak-moderate signal (normal for 1-min)")
    else:
        print(f"     ⚠️  Very weak signal (near random)")
    ...
    print(f"\n  🚀 Running Boruta (may take 10-30 minutes)...")
    print(f"  {'-'*76}")
    ...
    if len(confirmed) >= 40:
        ...
        print(f"     For RL: This is still usable, or skip Boruta entirely")
...
def create_report(df, feature_cols, confirmed, tentative, rejected, rankings):
...
def save_outputs(df, report_df, confirmed, tentative):
    ...
    if len(selected) > 0:
        ...
        print(f"  ✅ Selected dataset saved: {OUTPUT_SELECTED}")
...
def main():
    ...
    section("1-MINUTE BORUTA - OHLCV FEATURES ONLY")
    
    print(f"  📋 Configuration:")
    print(f"     Target:        {TARGET_VARIABLE}")
    print(f"     Sample size:   {SAMPLE_SIZE:,}")
    print(f"     Pre-filter:    Top {PREFILTER_TOP_N if PREFILTER_TOP_N else 'all'}")
    ...
    print(f"     - Reward = target_return_5min (not direction)")
    ...
    # Create report
    report_df = create_report(df, feature_cols, confirmed, tentative, rejected, rankings)
    ...
    if len(confirmed) >= 20:
        ...
        print(f"\n  🎯 Next: Consider using ALL technical features (skip Boruta)")
        print(f"     Transformer can learn which features matter")

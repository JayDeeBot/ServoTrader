"""
Download Latest 100,000 1m Candles for Training Dataset

This script fetches the latest 100,000 one-minute OHLCV candles
for all crypto codes listed in your JSON config, using the Binance API via ccxt.
...
for i, symbol in enumerate(crypto_codes):
    print(f"\n🚀 [{i+1}/{total_codes}] Fetching latest {MAX_CANDLES:,} 1m candles for {symbol}...")
    collected = []
    now = int(time.time() * 1000)
    since = now - MAX_CANDLES * 60_000
    ...
    while len(collected) < MAX_CANDLES:
        ...
        if len(raw) < LIMIT:
            break  # Prevent looping when max data has been reached

#!/usr/bin/env python3
"""
fetch_daily_historical_data.py

Fetch extended historical daily OHLCV data from Binance for prediction model training.

This script downloads daily candles going back as far as possible (2017 for BTC)
to provide sufficient training data for daily prediction models.

Key Features:
- Fetches daily (1d) candles by default
- Goes back to 2017-01-01 for maximum historical data
...
Usage:
    ...
    python fetch_daily_historical_data.py --symbol BTCUSDT
    
    # Fetch multiple symbols from JSON file
    python fetch_daily_historical_data.py --codes-json /path/to/crypto_codes.json
    
    # Specify output directory
    python fetch_daily_historical_data.py --symbol BTCUSDT --output-dir /path/to/output
...
# Binance API credentials (public endpoints don't require auth, but including for rate limits)
BINANCE_API_KEY = ""  # Optional - leave empty for public data
BINANCE_SECRET = ""   # Optional - leave empty for public data

# Default settings
DEFAULT_OUTPUT_DIR = "/home/jarred/git/ServoTrader/data/daily_historical"
DEFAULT_INTERVAL = '1d'  # Daily candles
DEFAULT_START_DATE = '2017-01-01T00:00:00Z'  # BTC listing on Binance
MAX_CANDLES = 100000  # More than enough for daily data (100k days = 273 years)
...
def fetch_symbol_data(binance, symbol: str, interval: str, start_date: str) -> pd.DataFrame:
    ...
    Args:
        ...
        start_date: Start date in ISO format (e.g., '2017-01-01T00:00:00Z')
    ...
    while len(collected) < MAX_CANDLES:
        try:
            ...
            if last_time >= now - timedelta(days=1):
                print(f"   ✅ Reached current date. Total: {len(collected):,} candles")
                break
            
            # Progress update every 1000 candles
            if len(collected) % 1000 == 0:
                print(f"   📊 {symbol}: {len(collected):,} candles | Last: {last_time.strftime('%Y-%m-%d')}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Fetch historical daily OHLCV data from Binance'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=f'Start date in ISO format (default: {DEFAULT_START_DATE})'
    )
    parser.add_argument(
    ...
    if not args.symbol and not args.codes_json:
        ...
        print("\nExamples:")
        print("  python fetch_daily_historical_data.py --symbol BTCUSDT")
        print("  python fetch_daily_historical_data.py --codes-json /path/to/codes.json")
        return
    ...
    print("=" * 60)
    print("🚀 ServoTrader Daily Historical Data Fetcher")
    print("=" * 60)
